...
function lowerCaseStrings(strings) {
  ...
  //return strings.map(strings => strings.toLowerCase());
...
function isItAnApple(strings) {
    let trueStrings = strings.map(function(item){
      if (item === "apple") {
        return true;
      }else {
        return false;
...
function removeApple(strings) {
  const changedArray = strings.filter(function(item){
    return item !== "apple";
  });
  return changedArray;
}
...
function stringSmash(strings) {
  let reduced = strings.reduce(function(string, item){
    //let comma = string.length ? '' : '';
    return string + item;
  });
  return reduced;
  //console.log(reduced);
...
function firstNamesAllCaps(runners) {
  ...
  runners.forEach(function(item){
    return upperFirstNames.push(item.first_name.toUpperCase());
  });
  return upperFirstNames;
}
 ...
 *
 *   -Counter1 is returning the function itself and the second counter2 is returning the count and not the actual function.
 *
 * 2. Which of the two uses a closure? How can you tell?
 *
 *   -Counter1 code uses closures because it has a nested function called counter() and this nested function has access to the parent scope.
 *
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better?
 *
 *   -When you do not want another function to have access to the variable, so it will not be able to be edited or changed.
*/
